...
@section('content_header')
    ...
    gap: 1px;
">
        <i class="fas fa-store"></i><i class="fas fa-tags"></i> Data Usaha Jenis
    </h1>
@stop

@section('content')
    <a href="{{ route('admin.usaha_jenis-create') }}" class="btn btn-success btn-sm">
        <i class="fas fa-add"></i> Tambah Usaha Jenis</a>
    {{-- tambahkan jarak dan garis --}}
    <br>
    <hr color="#ccc">
    {{-- tambahkan garis lurus --}}
    <table id="usaha_jenis-table" class="display" style="width:100%">
        <thead>
            <tr>
                <th>Nama Usaha</th>
                <th>Jenis Usaha</th>
        ...
        <tbody>
            @foreach ($usahaJeniss as $usahaJenis)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin.usaha_jenis-edit', $usahaJenis->id) }}" class="btn btn-warning btn-sm">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('admin.usaha_jenis-destroy', $usahaJenis->id) }}" method="POST"
